...
void main() {
  group('Publisher API', () {
    ...
    group('Invite a new member', () {
      Future<List<Map>> queryContents({String? email}) async {
      ...
      testWithProfile('Pending with Consent, sending new e-mail', fn: () async {
        ...
        expect(await queryContents(email: 'other@pub.dev'), [
          {
            'id': isNotNull,
            'fromAgent': adminUser.userId,
            'fromUserId': adminUser.userId,
      ...
      testWithProfile('Invite new account', fn: () async {
        ...
        expect(list.members.where((m) => m.email == 'newuser@example.com'),
            isEmpty);
        expect(await queryContents(email: 'newuser@example.com'), [
          {
            'id': isNotNull,
      ...
      testWithProfile('Invite existing account', fn: () async {
        ...
        expect(rs.emailSent, isTrue);
        expect(await queryContents(email: 'user@pub.dev'), [
          {
            'id': isNotNull,
            'fromAgent': user.userId,
            'fromUserId': user.userId,
